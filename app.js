...
function exportToXLSX() {
  ...
  ['Daily', 'Weekly', 'Monthly'].forEach(type => {
      ...
      const wscols = sheetHeaders.map(h => ({ wch: Math.max(15, Math.min(h.length, 50)) }));
      ws['!cols'] = wscols;

      XLSX.utils.book_append_sheet(wb, ws, `Checklist_${type}`);
  });
  
  const dateStr = new Date().toISOString().split('T')[0];
  XLSX.writeFile(wb, `Export_SHERPA_${dateStr}.xlsx`);
}
